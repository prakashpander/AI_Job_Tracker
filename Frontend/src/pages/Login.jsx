...
const Login = () => {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      // Direct Axios Post call to backend url
      const res = await axiosInstance.post(`/user/login`, { email, password }, config);
        ...
        <div className="rounded-2xl border border-card-border bg-card-bg p-8 shadow-xl backdrop-blur-md">
          ...
          <div className="mt-6 text-center">
            <p className="font-sans text-xs text-[#9ca3af]">
              ...
              <Link to="/register" className="font-semibold text-[#a78bfa] hover:text-[#7c3aed] hover:underline">
                Create account
              </Link>
            </p>
          </div>
